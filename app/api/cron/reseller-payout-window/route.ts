...
import { runResellerPayoutWindowJob } from '@/lib/cron-jobs';
...
export async function GET(req: Request) {
  ...
  try {
    const result = await runResellerPayoutWindowJob();
    return NextResponse.json({ success: true, ...result });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message || 'Reseller payout window run failed' }, { status: 500 });
  }
}
