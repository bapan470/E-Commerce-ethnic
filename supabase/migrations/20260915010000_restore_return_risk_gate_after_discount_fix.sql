...
--
-- 20260912120000_fix_discount_price_manipulation.sql redefined
-- place_order_with_items() starting from the 20260901000000
-- version of the function (per its own header comment: "Since
-- 20260901000000_online_payment_discount.sql, the function
-- already recomputes UNIT PRICE server-side...").
--
-- But 20260911000000_return_rto_risk_tracking.sql had ALREADY
-- added a COD block later than that, on 20260911 -- a check that
-- rejects Cash-on-Delivery orders from phone numbers inside their
-- 15-day return/RTO risk cooldown (raising
-- 'COD_BLOCKED_RETURN_RISK: ...').
--
-- Because CREATE OR REPLACE FUNCTION replaces the ENTIRE function
-- body, deploying 20260912120000 silently deleted that COD
-- risk-gate again -- it wasn't a deliberate revert, it's a case of
-- one migration being branched off an older copy of the function
-- than what was actually live.
--
-- This migration keeps 100% of 20260912120000's price-manipulation
-- fix (coupon / gift card / loyalty / online-payment discount are
-- still recomputed server-side, never trusted from the client) and
-- ADDS BACK the return/RTO risk COD gate from 20260911000000, so
-- neither fix undoes the other going forward.
--
-- NOTE: this assumes 20260911000000_return_rto_risk_tracking.sql
-- (which creates the `customer_return_risk` table) is already
-- applied to this database. If it is not, running this migration
-- will fail with a "relation customer_return_risk does not exist"
-- error -- run that migration first in that case.
-- ============================================================
...
DECLARE
  ...
  -- ---- return/RTO risk gate (restored from 20260911000000) ----
  v_phone text;
