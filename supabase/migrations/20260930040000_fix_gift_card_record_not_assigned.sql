-- ============================================================
-- Fix: "Failed to place order" on every checkout
--
-- place_order_with_items() declared `v_gift_card` as a generic
-- `record` and only assigns it when a gift card code is actually
-- applied (inside the gift-card IF branch). Near the end of the
-- function it then unconditionally referenced v_gift_card.id:
--
--   IF v_gift_card_discount > 0 AND v_gift_card.id IS NOT NULL THEN
--
-- In PL/pgSQL, referencing a field on a record that was never
-- assigned raises:
--   record "v_gift_card" is not assigned yet
--   (tuple structure of a not-yet-assigned record is indeterminate)
--
-- This fired on every order that did NOT use a gift card (i.e.
-- almost all orders), because v_gift_card was never populated,
-- yet .id was still referenced. The fix captures the gift card's
-- id into a plain uuid variable at the moment it's validated, and
-- uses that variable afterwards instead of touching the record.
-- ============================================================
...
BEGIN
  ...
  -- FIX: use v_gift_card_id (plain uuid, always safe to read) instead of
  -- v_gift_card.id (record field that is only assigned when a gift card
  -- was actually applied; referencing it otherwise raised
  -- "record \"v_gift_card\" is not assigned yet" and failed EVERY order).
